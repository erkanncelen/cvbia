...
def cv_text_to_yaml(cv_text:str=None, output_dir:str="../gpt/yaml_output", output_file_name:str=None):  
    yaml_text = ""
    text = cv_text
    ...
    about_me = gpt_communicator(text, questions['about_me']).replace('\n', ' ').replace('  ', ' ').strip()
    yaml_text += '\n' + f'about_me: "{about_me}"'
    ...
    for degree in education_degrees.split(','):
        degree = degree.strip()
        education_year = gpt_communicator(text, questions['education_year'].format(degree=degree)).strip()
        education_school = gpt_communicator(text, questions['education_school'].format(degree=degree)).strip()
        yaml_text += '\n' + f'  - degree: "{degree}"'
    ...
    biography = gpt_communicator(text, questions['biography']).replace('\n', ' ').replace('  ', ' ').strip()
    ...
    for role in roles.split(','):
        role = role.strip()
        role_description = gpt_communicator(text, questions['role_description'].format(role=role)).strip()
        yaml_text += '\n' + f'  - title: "{role}"'
    ...
    for competences_title in competences_titles.split(','):
        ...
        competences = gpt_communicator(text, questions['competences'].format(competences_title=competences_title)).strip()
    ...
    for company in companies.split(','):
        ...
        company_role = gpt_communicator(text, questions['company_role'].format(company=company)).strip()
        company_start = gpt_communicator(text, questions['company_start'].format(company=company)).strip()
        company_end = gpt_communicator(text, questions['company_end'].format(company=company)).strip()
        company_work = gpt_communicator(text, questions['company_work'].format(company=company)).strip().replace("\n", "\n      ").replace("¢", "•").replace("* ", "• ").replace("+ ", "• ").replace("« ", "• ")
        company_technologies = gpt_communicator(text, questions['company_technologies'].format(company=company)).strip()
